...
import com.withoutstress.ws_api.common.ActividadValidator;
    ...
    public Actividad finishActividad(Long actividadId){
        ...
        if(usuario.getNum_act_completas() % 5 == 0)
        {
            Recompensa recompensa = Recompensa.builder()
                    .nombre("Very Good")
                    .detalles("Sos grande N°" + (usuario.getNum_act_completas()/5))
                    .usuario(usuario)
                    .build();
        ...
        if(usuario.getNum_act_completas() % 15 == 0)
        {
            Recompensa recompensa = Recompensa.builder()
                    .nombre("Winner")
                    .detalles("El verdadero winner " + (usuario.getNum_act_completas()/73))
                    .usuario(usuario)
                    .build();

            usuario.setNum_monedas(usuario.getNum_monedas() + 666);
            recompensaRepository.save(recompensa);
        }
